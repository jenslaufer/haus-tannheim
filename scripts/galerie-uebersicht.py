...
Two positions are shown side by side so a photo can be referenced unambiguously:
  - "aktuelle"      = curated live order (index.html, the ``order`` array; _KWF2044 dropped)
  - "urspruengliche"= legacy/original order (index2.html: _KWF* then VS-Tannheim*, natural sort)
...
with open(csv_path, "w", newline="") as fh:
    ...
    w.writerow(["aktuelle_position", "urspruengliche_position", "dateiname", "status"])
    for n in sorted(files, key=lambda n: (new_pos.get(n, 10**9), legacy_pos.get(n, 0))):
        ap = new_pos.get(n)
        w.writerow([ap if ap else "", legacy_pos.get(n, ""), n,
                    "live" if n in new_pos else "nicht auf Live-Seite"])
print("CSV:", csv_path)
...
ordered = sorted(files, key=lambda n: (new_pos.get(n, 10**9), legacy_pos.get(n, 0)))
...
for pi in range(total_pages):
    ...
    d.text((MARGIN, 68),
           "Pos = aktuelle Galerie-Position · (urspr. N) = ursprüngliche/Legacy-Reihenfolge",
    ...
    for idx, n in enumerate(ordered[pi*per_page:(pi+1)*per_page]):
        ...
        ap = new_pos.get(n)
        label = (f"Pos {ap}" if ap else "— (nicht live)")
        if legacy_pos.get(n):
            label += f"  (urspr. {legacy_pos[n]})"
